...
object Message {
  ...
  def parse(line: String, messageStartPosition: Int = 3) = {
    ...
    var msg = ""
    val problemType = if (report(2).trim == "warning") "Warning" else "Error"
    ...
    new Message(name, msg, report(0).substring(report(0).lastIndexOf(separator) + 1), report(1).trim, projectName + separator + report(0), problemType)
  }
}