  ...
  final class LexerNode {
    ...
    def parse(buffer: CharBuffer, offset: Int, length: Int) {
      ...
      while (matchEnd < checkPoint && matcher.find(pointer)) {
        ...
        children.get(groupList(groupNum)) foreach { childNode =>
...
object FilterFactory {
  def build(name: String, doc: DefaultStyledDocument): DocumentFilter = {
    var filter: DocumentFilter = null
    if (name != null && (name.toLowerCase.endsWith(".scala") || name.toLowerCase.endsWith(".java"))) {
      filter = new ScalaFilter(doc)
    } else if (name != null && (name.toLowerCase.endsWith(".properties") || name.toLowerCase.endsWith(".conf"))) {
